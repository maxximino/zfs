...
static inline int posix_acl_chmod(struct posix_acl** acl, int flags, umode_t umode){
	...
	*acl = posix_acl_clone(*acl, flags);
	...
	err = posix_acl_chmod_masq(*acl, mode);
	if (err) {
...
static inline int posix_acl_create(struct posix_acl** acl, int flags, umode_t* umodep){
	struct posix_acl *oldacl = *acl;
	mode_t mode = *umodep;
	int err;
	*acl = posix_acl_clone(*acl, flags);
   	zpl_posix_acl_release(oldacl);
	if (!(*acl)) { return -ENOMEM; }
	err = posix_acl_create_masq(*acl, &mode);
	*umodep = mode;
	if (err < 0) {
	    zpl_posix_acl_release(*acl);
            *acl = NULL;
