...
zpl_xattr_set(struct inode *ip, const char *name, const void *value,
    size_t size, int flags, boolean_t use_sa)
{
	znode_t *zp = ITOZ(ip);
	zfs_sb_t *zsb = ZTOZSB(zp);
	cred_t *cr = CRED();
	...
	if (zsb->z_use_sa && (zsb->z_xattr_sa || use_sa) && zp->z_is_sa) {
    ...
    const void *value, size_t size, int flags)
{
	...
	xattr_name = kmem_asprintf("%s%s", XATTR_USER_PREFIX, name);
	error = zpl_xattr_set(ip, xattr_name, value, size, flags, B_FALSE);
	strfree(xattr_name);
...
zpl_set_acl(struct inode *ip, int type, struct posix_acl *acl)
{
	...
	/*
	 * Preferentially write the xattr as a SA using the same logic as
	 * if xattr=sa were set.  Posix ACLs are Linux-only and if they
	 * can't be seen from other platforms that's not an issue.  It may
	 * even be preferable since they are not understood/used by them.
	 */
	error = zpl_xattr_set(ip, name, value, size, 0, B_TRUE);
	if (value)
		kmem_free(value, size);
...
#ifndef HAVE_CHECK_ACL
int zpl_permission(struct inode* ip, int mask){
	return generic_permission(ip,mask,zpl_check_acl);
}
